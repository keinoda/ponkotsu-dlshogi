...
# 変数設定
save_dir=$1
name="resnet10x154_llama4_experts_system"
...
for ((i=$start; i<=$last; i++)); do
    ...
    python -m dlshogi.train ${src} ${data_dir}/floodgate_test_2017-2018_r3500_eval5000.hcpe\
     ${resume} --checkpoint ${checkpoint} --network policy_value_network_experts_system.PolicyValueNetwork --model ${model} -e 1\
    --use_average --use_evalfix ${use_swa} --use_amp --temperature 0 --lr 0.2\
    --lr_scheduler ReduceLROnPlateau'('eps=1e-20,factor=0.5')' --cache ${cache_dir}/train_cache_experiment_${kkk} --log ${log_dir}/train_log.txt
