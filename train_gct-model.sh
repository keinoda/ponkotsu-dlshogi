...
# 変数設定
save_dir=$1
name="densenet10_g32_c192_add_options_kernel3_adding_gct_model"
...
for ((i=$start; i<=$last; i++)); do
    ...
    if [ $i -eq 365 ]; then
        resume="-r ${checkpoint_dir}/checkpoint_densenet10_g32_c192_add_options_kernel3_adding_gct-364.pth"
        lr="${lr} --reset_scheduler"
    else
